...
- `repositories`: Repositories use services to fetch, query, and update data models. There is generally one repository per data model type. They do not typically maintain any public state, and do not communicate with each other, as orchestration and caching should be performed in view models or application-level notifiers. Where applicable, repositories may return a stream of data for read operations, where the stream emits new values as the data is updated.  
- `application`: The application folder contains files for maintaining application-wide (global) state for data types where the dataset is both used across multiple pages, and small enough to be reasonable to keep the entire dataset cached in memory. 
